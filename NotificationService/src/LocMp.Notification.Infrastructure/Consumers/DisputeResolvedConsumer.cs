...
public sealed class DisputeResolvedConsumer(NotificationDbContext db, IDistributedCache cache)
    : IConsumer<DisputeResolvedEvent>
{
    public async Task Consume(ConsumeContext<DisputeResolvedEvent> ctx)
    {
        ...
        var (buyerBody, sellerBody) = msg.Outcome switch
        {
            DisputeOutcome.BuyerFavored =>
                ("Спор решён в вашу пользу. Заказ отменён.", "Спор решён в пользу покупателя."),
            DisputeOutcome.SellerFavored =>
                ("Спор решён в пользу продавца. Заказ засчитан как выполненный.", "Спор решён в вашу пользу. Заказ засчитан."),
            _ => ("Спор по заказу был разрешён.", "Спор по заказу был разрешён.")
        };
