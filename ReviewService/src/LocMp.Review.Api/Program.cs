...
using LocMp.Review.Application.Extensions;
using LocMp.Review.Infrastructure.Extensions;
...
try
{
    ...
    Log.Information("ReviewService started successfully");

    app.Run();
}
catch (Exception ex) when (ex is not OperationCanceledException && ex.GetType().Name != "HostAbortedException")
{
    Log.Fatal(ex, "ReviewService terminated unexpectedly");
}
finally
{
    Log.CloseAndFlush();
